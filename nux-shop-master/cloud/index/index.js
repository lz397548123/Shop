...
const returnUtil = require('utils/ReturnUtil.js')  //返回结果的工具
...
// const IMAGEPREFIX = "cloud://yunkaifa-xuexi-1.7975-yunkaifa-xuexi-1-1301233624"
...
// 获取最新爆款
   app.router('getProductNew', async (ctx, next) => {
    ctx.data = await _productItem(product.getnewProduct({}, 0, 4))
    ctx.body = await returnUtil.success(ctx)
    await next()
  })
    ...
    return new Promise((resolve, reject) => {
      data.then(res => {
        res.data.forEach(data => {
          ...
          data.theme_icon = data.theme_icon
        })
        resolve(res)
      })
    })
  ...
  function _productImg(data) {
    return new Promise((resolve, reject) => {
      data.then(res => {
        ...
        res.data.product_img =  res.data.product_img
        resolve(res)
      })
    })
  }
